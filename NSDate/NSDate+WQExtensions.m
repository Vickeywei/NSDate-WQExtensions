...
@implementation NSDate (WQExtensions)
...
-(NSString *)formatterModel2
{
    ...
    if(time>=secondsOneYear){
        ...
        return [NSString stringWithFormat:@"%@个月前",months[month - 1]];
    }else if (time>=secondsOneWeek){
        int week = time/secondsOneWeek;
        NSArray * weeks= @[@"一",@"二",@"三",@"四"];
        return [NSString stringWithFormat:@"%@周前",weeks[week - 1]];
...
{
    if ([NSString stringWithFormat:@"%lld",time].length > 10) {
        time = time / 1000;
    }
